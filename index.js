      ...
      <div class="overflow-x-auto">
        <table class="w-full divide-y divide-gray-200 responsive-table">
          <thead class="bg-gray-50">
            <tr>
              <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider" style="width: 22%;">
                名称
              </th>
...
const lunarBiz = {
  ...
  daysToLunar(lunar) {
    ...
    function showRenewFormModal(subscription) {
            ...
            <div id="renewFormModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50" onclick="closeRenewFormModal(event)">
                <div class="relative top-20 mx-auto p-5 border w-full max-w-md shadow-lg rounded-md bg-white" onclick="event.stopPropagation()">
                    ...
                    <form id="renewForm" class="mt-4 space-y-4">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">新到期日（可选）</label>
                            <input type="date" id="renewCustomExpiryDate"
                                   class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500"
                                   oninput="updateNewExpiryPreview()">
                            <label class="mt-2 inline-flex items-center text-sm text-gray-600">
                                <input type="checkbox" id="renewUsePaymentAsExpiry" class="form-checkbox h-4 w-4 text-indigo-600 mr-2">
                                使用支付日期作为新到期日
                            </label>
                        </div>
        ...
        document.getElementById('renewPaymentDate').addEventListener('input', updateNewExpiryPreview);
        document.getElementById('renewCustomExpiryDate').addEventListener('input', updateNewExpiryPreview);
        document.getElementById('renewUsePaymentAsExpiry').addEventListener('change', updateNewExpiryPreview);
    }
    ...
    function updateNewExpiryPreview() {
        ...
        const paymentDateField = document.getElementById('renewPaymentDate');
        const customExpiryField = document.getElementById('renewCustomExpiryDate');
        const paymentDateValue = paymentDateField.value;
        const usePaymentAsExpiry = document.getElementById('renewUsePaymentAsExpiry').checked;

        if (usePaymentAsExpiry) {
            customExpiryField.value = paymentDateValue;
            customExpiryField.disabled = true;
        } else {
            customExpiryField.disabled = false;
        }

        const customExpiryValue = customExpiryField.value;
        const customExpiryDate = usePaymentAsExpiry ? paymentDateValue : customExpiryValue;
        const customExpiryPreview = customExpiryDate ? new Date(customExpiryDate) : null;

        if (customExpiryPreview && !isNaN(customExpiryPreview.getTime())) {
            document.getElementById('newExpiryPreview').textContent = customExpiryPreview.toLocaleDateString('zh-CN');
            return;
        }
    ...
    async function handleRenewFormSubmit(e) {
        ...
        const paymentDateValue = document.getElementById('renewPaymentDate').value;
        const customExpiryValue = document.getElementById('renewCustomExpiryDate').value;
        const usePaymentAsExpiry = document.getElementById('renewUsePaymentAsExpiry').checked;
        const customExpiryDate = usePaymentAsExpiry ? paymentDateValue : customExpiryValue;

        const options = {
            paymentDate: paymentDateValue,
            nextExpiryDate: customExpiryDate || undefined,
...
async function manualRenewSubscription(id, env, options = {}) {
  try {
    ...
    if (options.nextExpiryDate) {
      const manualExpiry = new Date(options.nextExpiryDate);
      if (isNaN(manualExpiry.getTime())) {
        return { success: false, message: '新到期日期格式无效' };
      }
      newExpiryDate = manualExpiry;
    }
